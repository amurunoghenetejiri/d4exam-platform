 ...
 * Fail-soft: never crashes the build if parts are missing/corrupt.
 */
...
function expand(prefix, targetRel) {
  try {
    ...
    if (!parts.length) {
      console.log("[restore] skip", targetRel, "(no parts)");
      return false;
    }
    ...
    console.warn("[restore] failed", targetRel, String(e && e.message ? e.message : e));
    return false;
  }
}
