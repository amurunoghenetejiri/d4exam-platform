-- Flexible application lookup / claim for password setup
-- (case-insensitive tracking code; match applicant OR issued admin email)
...
DECLARE
  ...
  v_code text := lower(trim(coalesce(_tracking_code, '')));
BEGIN
  RETURN QUERY
  SELECT
    a.id,
  ...
  WHERE lower(trim(coalesce(a.tracking_code, ''))) = v_code
    AND (
      lower(trim(coalesce(a.applicant_email, ''))) = v_email
      OR lower(trim(coalesce(a.issued_admin_email, ''))) = v_email
    )
...
DECLARE
  ...
  v_track text := lower(trim(coalesce(_tracking_code, '')));
BEGIN
  ...
  SELECT * INTO v_app
  FROM public.school_applications a
  WHERE lower(trim(coalesce(a.tracking_code, ''))) = v_track
    AND (
      lower(trim(coalesce(a.applicant_email, ''))) = v_email
      OR lower(trim(coalesce(a.issued_admin_email, ''))) = v_email
    )
