-- Broader staff login matching: staff_id, full_name, or profile email
...
BEGIN
  ...
  -- direct email match on profiles
  RETURN QUERY
  ...
  -- teacher: staff_id, full_name, or profile email
  RETURN QUERY
  ...
  WHERE t.school_id = v_school_id
    AND (
      ...
      OR lower(regexp_replace(trim(coalesce(t.full_name,'')), '\s+', ' ', 'g')) = v_ident
