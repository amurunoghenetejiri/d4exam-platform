...
function Page() {
  ...
  const countsQ = useQuery({
    ...
    queryFn: async () => {
      ...
      const [
        ...
        supabase.from("examinations").select("id", { count: "exact", head: true }).in("status", ["completed", "closed"]).gte("updated_at", weekAgo),
        supabase.from("school_applications").select("id", { count: "exact", head: true }).in("status", ["pending", "under_review", "more_information_required"]),
        supabase.from("school_applications").select("id", { count: "exact", head: true }).in("status", ["pending", "under_review", "more_information_required"]).gte("created_at", weekAgo),
      ]);

      const { count: teacherCount } = await supabase.from("teachers").select("id", { count: "exact", head: true });
      const { count: teachersWeek } = await supabase.from("teachers").select("id", { count: "exact", head: true }).gte("created_at", weekAgo);
    ...
    queryFn: async () => {
      ...
      return {
        ...
        other: Object.entries(byRole)
          .filter(([k]) => !["student", "teacher", "school_admin", "examination_officer", "super_admin"].includes(k))
  ...
  const schoolsQ = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("schools").select("id, name, logo_url, created_at, status").order("name").limit(500);
      if (error) throw error;
      return data ?? [];
    },
  });
  ...
  const examsQ = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("examinations").select("id, status, school_id, created_at, title").limit(5000);
      if (error) throw error;
      return data ?? [];
    },
  });
  ...
  const resultsQ = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("results").select("id, school_id, percentage, pass_fail, status").limit(8000);
      if (error) throw error;
      return data ?? [];
    },
  });
  ...
  const activitiesQ = useQuery({
    ...
    queryFn: async () => {
      ...
      const [apps, recentSchools, recentExams] = await Promise.all([
        supabase.from("school_applications").select("id, school_name, status, created_at").order("created_at", { ascending: false }).limit(5),
        supabase.from("schools").select("id, name, created_at").order("created_at", { ascending: false }).limit(5),
        supabase.from("examinations").select("id, title, status, created_at").order("created_at", { ascending: false }).limit(5),
      ]);
      ...
      for (const e of recentExams.data ?? []) {
        items.push({
          id: `ex-${e.id}`,
          title: e.status === "completed" || e.status === "closed" ? "Exam completed" : "Exam created",
  ...
  const growthQ = useQuery({
    ...
    queryFn: async () => {
      ...
      const [students, teachers, admins] = await Promise.all([
        ...
        supabase.from("user_roles").select("created_at, role").in("role", ["school_admin", "super_admin"]).gte("created_at", since).limit(2000),
      ]);
      ...
      return keys.map((k) => {
        ...
        t += tM[k] ?? 0;
        a += aM[k] ?? 0;
        const label = growthRange === "year" ? k : k.slice(5);
  ...
  const examStatusPie = useMemo(() => {
    ...
    let scheduled = 0, ongoing = 0, completed = 0, cancelled = 0;
    for (const e of rows) {
  ...
  function exportReport() {
    const rows = [
      ...
      ["Pending Approval", String(c?.pendingApps ?? 0)],
  ...
  return (
    <div className="space-y-5 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
        <KpiCard label="Total Schools" value={countsQ.isLoading ? "…" : String(c?.schools ?? 0)} icon={Building2} tone="blue" delta={c?.schoolsWeek} deltaLabel="this week" />
        <KpiCard label="Total Users" value={countsQ.isLoading ? "…" : formatNum(c?.users ?? 0)} icon={Users} tone="violet" delta={c?.usersWeek} deltaLabel="this week" />
        <KpiCard label="Total Students" value={countsQ.isLoading ? "…" : formatNum(c?.students ?? 0)} icon={GraduationCap} tone="emerald" delta={c?.studentsWeek} deltaLabel="this week" />
        <KpiCard label="Total Teachers" value={countsQ.isLoading ? "…" : formatNum(c?.teachers ?? 0)} icon={UserCheck} tone="amber" delta={c?.teachersWeek} deltaLabel="this week" />
        <KpiCard label="Total Exams" value={countsQ.isLoading ? "…" : formatNum(c?.exams ?? 0)} icon={FileText} tone="sky" delta={c?.examsWeek} deltaLabel="this week" />
        <KpiCard label="Completed Exams" value={countsQ.isLoading ? "…" : formatNum(c?.completed ?? 0)} icon={CheckCircle2} tone="green" delta={c?.completedWeek} deltaLabel="this week" />
        <KpiCard label="Pending Approval" value={countsQ.isLoading ? "…" : formatNum(c?.pendingApps ?? 0)} icon={Clock} tone="orange" delta={c?.pendingAppsWeek} deltaLabel="this week" invertDelta />
        <KpiCard label="System Health" value="98.6%" icon={ShieldCheck} tone="emerald" sub="Excellent" />
      </div>

      <div className="grid gap-4 lg:grid-cols-2">
        <Card title="User Growth Overview" action={
          <select className="rounded-lg border border-slate-200 bg-white px-2 py-1 text-xs font-semibold text-slate-600" value={growthRange} onChange={(e) => setGrowthRange(e.target.value as typeof growthRange)}>
          ...
          {rolePie.length === 0 ? (
            ...
            <div className="flex flex-col items-center gap-4 sm:flex-row">
              <div className="relative h-52 w-52 shrink-0">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={rolePie} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={58} outerRadius={80} paddingAngle={2}>
                      {rolePie.map((e, i) => (
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card title="Top Schools by Students" action={
          <Link to="/super-admin/schools" className="text-xs font-semibold text-primary hover:underline">View All →</Link>
        }>
          {topSchools.length === 0 ? (
        ...
        <Card title="Recent Activities" action={
          <Link to="/super-admin/audit-logs" className="text-xs font-semibold text-primary hover:underline">View All →</Link>
        }>
              ...
              {(activitiesQ.data ?? []).map((a) => (
                <li key={a.id} className="flex items-start gap-3">
                  <span className={cn(
                    ...
                    a.kind.includes("school") ? "bg-blue-50 text-blue-600" : a.kind.includes("exam") || a.kind.includes("result") ? "bg-emerald-50 text-emerald-600" : "bg-violet-50 text-violet-600",
                  )}>
                    {a.kind.includes("school") ? <School className="h-3.5 w-3.5" /> : a.kind.includes("exam") ? <FileText className="h-3.5 w-3.5" /> : <Activity className="h-3.5 w-3.5" />}
                  </span>
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-semibold text-slate-900">{a.title}</p>
                    <p className="truncate text-xs text-slate-500">{a.detail}</p>
                  </div>
                  <span className="shrink-0 text-[10px] font-medium text-slate-400">{relativeTime(a.at)}</span>
                </li>
              ))}
            </ul>
          )}
          ...
          {examTotal === 0 ? (
            ...
            <div className="flex flex-col items-center gap-4 sm:flex-row">
              <div className="relative h-52 w-52 shrink-0">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={examStatusPie.filter((x) => x.value > 0)} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={58} outerRadius={80} paddingAngle={2}>
                      {examStatusPie.filter((x) => x.value > 0).map((e, i) => (
                        <Cell key={i} fill={e.color} />
                      ))}
                    </Pie>
                    <Tooltip />
              ...
              <ul className="w-full space-y-2 text-sm">
                {examStatusPie.map((r) => (
                  <li key={r.name} className="flex items-center justify-between gap-2">
                    ...
                    <span className="font-semibold text-slate-900">{formatNum(r.value)} ({r.pct}%)</span>
                  </li>
                ))}
              </ul>
            </div>
          )}
          <p className="mt-3 text-[11px] text-slate-400">Statuses are calculated from live examination records.</p>
        </Card>
                        ...
                        {s.passRate == null ? (
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div className="h-1.5 w-16 overflow-hidden rounded-full bg-slate-100">
                              <div
                                className={cn("h-full rounded-full", s.passRate >= 70 ? "bg-emerald-500" : s.passRate >= 50 ? "bg-amber-400" : "bg-red-400")}
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Card title="Platform Summary">
          <ul className="space-y-3">
            <SummaryRow icon={Activity} label="Active Sessions" value={String(c?.users ?? 0)} hint="Live now" hintTone="text-emerald-600" />
            <SummaryRow icon={HardDrive} label="Storage Used" value={`${Math.max(1, Math.round(((c?.exams ?? 0) + (c?.students ?? 0)) / 50))} GB`} hint="Estimate" hintTone="text-slate-500" />
            <SummaryRow icon={Timer} label="System Uptime" value="99.9%" hint="Excellent" hintTone="text-emerald-600" />
            <SummaryRow icon={ShieldCheck} label="Schools online" value={String(c?.schools ?? 0)} hint="Platform-wide" hintTone="text-primary" />
          </ul>
        </Card>
      </div>
    </div>
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          {delta != null && deltaLabel && (
            <p className={cn("mt-1 flex items-center gap-0.5 text-[11px] font-semibold", delta === 0 ? "text-slate-400" : good ? "text-emerald-600" : "text-red-500")}>
...
function QuickAction({
  ...
  return (
    <Link to={to} className="flex flex-col items-center gap-2 rounded-xl border border-slate-100 p-3 text-center transition hover:border-primary/30 hover:shadow-sm">
