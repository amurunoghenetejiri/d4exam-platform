...
function Page() {
  ...
  return (
    <>
      <PageHeader
        title="Student list"
        description="All students in your school. Import or add new ones under Student Import."
        actions={
      ...
      <SectionCard title="Login reminder for students">
      ...
      <div className="mt-6">
        <SectionCard title={`Students (${(list.data ?? []).length})`}>
          {(list.data ?? []).length === 0 ? (
            <EmptyState
              title="No students yet"
              description="Go to Student Import to add one student or upload a CSV from Excel."
            />
          ) : (
