...
function Page() {
  ...
  function parseStudentCsv(text: string) {
    ...
    const nameIdx = header.findIndex((h) => /full\s*name|student\s*name|name/.test(h));
    const matricIdx = header.findIndex((h) => /matric|student\s*id|admission/.test(h));
  ...
  async function importStudentsFile(file: File) {
    ...
    try {
      ...
      toast.success(`Imported ${created} student(s)${skipped ? `, skipped ${skipped}` : ""}`);
      setShowAddStudent(false);
      await studentsQ.refetch();
    } catch (e) {
      toast.error((e as Error).message);
  ...
  if (!facultyId) {
    ...
    return (
      <>
        <PageHeader title="Academic Structure" description="Faculty / College → Department → Level → Courses & Students" />
        ...
        <div className="mt-4 grid gap-6 lg:grid-cols-[300px_minmax(0,1fr)]">
          <SectionCard title={editId ? "Edit" : "Add Faculty / College"}>
            <div className="space-y-3">
              <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Faculty of Engineering" />
          ...
          <SectionCard title={`Faculty / College (${list.length})`}>
            {facultiesQ.isLoading ? (
              ...
              <ul className="space-y-2">
                {list.map((f) => (
                  <li key={f.id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-slate-100 p-3">
                    ...
                    <div className="flex items-center gap-1">
                      <Button size="sm" variant="ghost" onClick={() => { setEditId(f.id); setName(f.name); setCode(f.code ?? ""); }}>
      ...
      <>
        ...
        <div className="mt-4 grid gap-6 lg:grid-cols-[300px_minmax(0,1fr)]">
          <SectionCard title={editId ? "Edit department" : "Add department"}>
            <div className="space-y-3">
              <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Computer Engineering" />
          ...
          <SectionCard title={`Departments (${list.length})`}>
            {departmentsQ.isLoading ? (
              ...
              <ul className="space-y-2">
                {list.map((d) => (
                  <li key={d.id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-slate-100 p-3">
                    ...
                    <div className="flex items-center gap-1">
                      <Button size="sm" variant="ghost" onClick={() => { setEditId(d.id); setName(d.name); setCode(d.code ?? ""); }}>
                        <Pencil className="h-3.5 w-3.5" />
                      </Button>
  ...
  if (departmentId && !levelId) {
    ...
    return (
      <>
        <PageHeader title={department?.name ?? "Department"} description="Choose a level. Courses and students live inside each level." />
        <Breadcrumbs items={crumbs} />
        <Toolbar search={q} onSearch={setQ} />
        <SectionCard
          className="mt-4"
        ...
        >
          <div className="mb-4 flex flex-wrap gap-2">
            <Input className="max-w-[200px]" placeholder="e.g. 200 Level" value={name} onChange={(e) => setName(e.target.value)} />
            <Input className="w-24" placeholder="200" value={code} onChange={(e) => setCode(e.target.value)} />
  ...
  return (
    <>
      ...
      <SectionCard
        ...
        description="Only courses for this level."
        action={
          <Button size="sm" variant={showAddCourse ? "secondary" : "outline"} className="font-semibold" onClick={() => setShowAddCourse((v) => !v)}>
      ...
      >
        {showAddCourse && (
          <div className="mb-4 flex flex-wrap items-end gap-2 rounded-xl border border-slate-100 bg-slate-50 p-3">
            ...
            <div className="min-w-[180px] flex-1 space-y-1">
              ...
              <Input value={cName} onChange={(e) => setCName(e.target.value)} placeholder="Introduction to Computer Engineering" />
            </div>
        ...
        {levelCoursesQ.isLoading ? (
          ...
          <EmptyState title="No courses for this level" description="Click Add course." icon={BookOpen} />
        ) : (
          <ul className="space-y-2">
            {levelCourses.map((c) => (
              <li key={c.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-slate-100 px-3 py-2 text-sm">
                <span>
      ...
      <SectionCard
        ...
        action={
          <Button size="sm" className="font-semibold" variant={showAddStudent ? "secondary" : "default"} onClick={() => setShowAddStudent((v) => !v)}>
            <Plus className="mr-1 h-3.5 w-3.5" />
            Add student
      ...
      >
        ...
        {studentsQ.isLoading ? (
          ...
          <EmptyState title="No students yet" description="Use Add student or Import CSV." icon={GraduationCap} />
        ) : (
                    ...
                    <tr key={s.id} className="border-b border-slate-50">
                      ...
                      <td className="py-2.5">
                        <Button
                          ...
                          onClick={() => void toggleStudentStatus(s)}
                        >
                          {suspended ? (
                            <>
