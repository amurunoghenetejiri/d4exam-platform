...
function CbtExamPage() {
  ...
  const warnAt =
    (settingsQ.data as { warning_threshold?: number } | null)?.warning_threshold ?? 3;
  ...
  useEffect(() => {
    ...
    async function runChecks() {
      ...
      notes.push(`Browser: ${ua.slice(0, 80)}…`);
  ...
  // Optional FaceDetector (Chrome)
  useEffect(() => {
    if (phase !== "exam" || !cameraStream || !security.requireCamera) return;
    let alive = true;
    const FaceDetectorCtor = (window as unknown as { FaceDetector?: new () => { detect: (s: HTMLVideoElement) => Promise<{ boundingBox: unknown }[]> } }).FaceDetector;
    if (!FaceDetectorCtor || !videoRef.current) {
      setFaceLabel("Camera active (face API unavailable)");
      return;
    }
    ...
    const tick = async () => {
      ...
      try {
        ...
        if (faces.length === 0) {
          ...
          setFaceLabel("One face detected");
        } else {
          setFaceLabel(`Multiple faces (${faces.length})`);
          void logEvent("MULTIPLE_FACES", "high", `${faces.length} faces detected`);
        }
      } catch {
        setFaceLabel("Camera active");
      }
    };
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [phase, cameraStream, security.requireCamera]);
  ...
  // Tab / fullscreen / copy protection
  useEffect(() => {
        ...
        <div className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          {cameraStream && (
            <div className="mt-4 overflow-hidden rounded-xl border border-emerald-200 bg-emerald-50">
              ...
              <p className="px-2 py-1 text-[11px] font-semibold text-emerald-800">
                Camera active — monitoring may record face presence events for review (not automatic
                cheating decisions).
              </p>
            </div>
          )}
    ...
    >
      ...
      <div className="mx-auto grid w-full max-w-[1200px] flex-1 gap-4 p-3 sm:p-6 lg:grid-cols-[220px_minmax(0,1fr)]">
        <aside className="order-2 space-y-3 lg:order-1">
          ...
          {security.requireCamera && cameraStream && (
            <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              <video ref={videoRef} muted playsInline className="h-24 w-full object-cover" />
              <p className="px-2 py-1.5 text-[10px] font-semibold text-slate-600">{faceLabel}</p>
            </div>
          )}
        </aside>
