...
function Page() {
  ...
  const filtered = useMemo(() => {
    ...
    return cards.filter((c) => {
      ...
      return (
        ...
        c.matric.toLowerCase().includes(q) ||
  ...
  const selectedTimeline = useMemo(() => {
    ...
    return events
      .filter(
        (e) =>
          e.student_id === selected.a.student_id &&
          (!e.exam_id || e.exam_id === selected.a.exam_id),
      )
      .slice(0, 20);
  }, [events, selected]);
                ...
                <>
                  ...
                  <div className="flex h-full flex-col items-center justify-center gap-2 px-4 text-center text-white">
                    ...
                    <p className="text-[11px] text-white/70">
                      {selected.isDone
                        ? "This student finished. Card leaves the grid after 3 minutes."
                        : selected.presence.cameraActive
...
function StudentCard({
  ...
  return (
    ...
    >
      <div className="relative aspect-[5/4] bg-gradient-to-br from-slate-800 to-slate-900 sm:aspect-[4/3]">
        {frameSrc ? (
        ...
        <span className="absolute left-1 top-1 inline-flex items-center gap-0.5 rounded-full bg-black/55 px-1 py-0.5 text-[8px] font-bold uppercase text-white sm:left-1.5 sm:top-1.5 sm:gap-1 sm:px-1.5 sm:text-[9px]">
          ...
          />
          {isDone ? "Done" : streamLive ? "Live" : isOnline(presence.lastSeenAt) ? "Live" : "Off"}
        </span>
        {!isDone && (
