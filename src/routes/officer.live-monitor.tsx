              ...
              {selected.hasLiveVideo && selected.frame?.src ? (
                ...
                <>
                  ...
                  <div className="flex h-full flex-col items-center justify-center gap-2 px-4 text-center text-white">
                    {selected.isDone ? <CheckCircle2 className="h-12 w-12 text-emerald-400/80" /> : selected.presence.cameraActive ? <UserRound className="h-12 w-12 opacity-40" /> : <CameraOff className="h-12 w-12 opacity-40" />}
                    <p className="text-sm font-semibold">{selected.isDone ? doneStatusLabel(selected.a.status) : faceLabel(selected.presence)}</p>
                    <p className="text-[11px] text-white/70">{selected.isDone ? "This student finished. Card leaves the grid after 3 minutes." : selected.presence.cameraActive ? "Live frames appear when the student camera is streaming. Nothing is recorded." : "Student camera is off or unavailable. Offline cards leave after 3 minutes."}</p>
                  </div>
                </>
              )}
            </div>
  ...
  return (
    <button type="button" onClick={onClick} className={cn("overflow-hidden rounded-lg border bg-white text-left shadow-sm transition hover:shadow-md sm:rounded-xl sm:border-2", severityBorderClass(sev))}>
      <div className={cn("relative aspect-[5/4] sm:aspect-[4/3]", isDone ? "bg-gradient-to-br from-sky-800 via-slate-800 to-slate-900" : "bg-gradient-to-br from-slate-800 to-slate-900")}>
        {isDone ? (
          <div className="flex h-full flex-col items-center justify-center gap-1.5 bg-sky-950/40 px-2 text-center">
            <CheckCircle2 className="h-8 w-8 text-sky-300 sm:h-10 sm:w-10" />
            <p className="text-[10px] font-bold uppercase tracking-wide text-sky-100 sm:text-[11px]">{statusLabel || "Submitted"}</p>
          </div>
        ) : frameSrc ? (
