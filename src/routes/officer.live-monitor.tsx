...
function Page() {
  ...
  const framesRef = useRef<Record<string, FrameEntry>>({});
  ...
  useEffect(() => {
    ...
    const sub = startLiveCamSubscriber({
      ...
      onFrame: (p: LiveCamFramePayload) => {
        ...
        if (selectedIdRef.current === p.attemptId) {
    ...
    return () => {
      sub.stop();
      if (frameFlushRef.current != null) {
        window.clearTimeout(frameFlushRef.current);
        frameFlushRef.current = null;
      }
    };
  }, [schoolId]);
  ...
  useEffect(() => {
    ...
    setFrames((prev) => {
      ...
      if (changed) framesRef.current = next;
  ...
  const alerts = useMemo(() => {
    return events
      .filter((e) => {
        ...
        return (
          t.includes("FACE") || t.includes("CAMERA") || t.includes("TAB") || t.includes("FULLSCREEN") ||
          t.includes("CONNECTION") || t.includes("SUBMIT") || t.includes("WARNING") ||
          t.includes("RESULT") || t.includes("TERMINAT") ||
  ...
  async function sendOfficerWarning() {
    ...
    const cooldownKey = `${selected.a.student_id}:${selected.a.exam_id}`;
    const lastSent = warningCooldownRef.current.get(cooldownKey) ?? 0;
    if (Date.now() - lastSent < 45_000) {
      toast.message("Warning already sent. Wait a moment before sending again.");
      return;
    }
    setWarningBusy(true);
    warningCooldownRef.current.set(cooldownKey, Date.now());
    try {
      await logSecurityEvent({
        ...
        severity: "high",
        description: "⚠️ Warning: Follow exam rules. Further violations may void your result.",
      ...
      await notifyStudentOfficerWarning({
        ...
        message: "⚠️ Warning: Follow exam rules. Further violations may void your result.",
      });
      toast.success(`Warning sent to ${selected.name}`);
      void eventsQ.refetch();
    } catch (e) {
      warningCooldownRef.current.delete(cooldownKey);
  ...
  const FILTERS = [["all", "All"], ["normal", "Normal"], ["warning", "Warn"], ["violation", "Viol."], ["offline", "Off"]] as const;
    ...
    <div className="mx-auto w-full max-w-[1400px] px-0 sm:px-0">
      ...
      <div className="mb-3 space-y-2 sm:mb-4">
        <div className="relative w-full sm:max-w-xs">
          ...
          <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search name or matric…" className="h-8 pl-8 text-xs sm:h-9 sm:text-sm" />
        </div>
        <div className="flex flex-wrap items-center gap-1.5">
          <div className="flex min-w-0 flex-1 flex-wrap gap-1">
            {FILTERS.map(([k, label]) => (
              <button key={k} type="button" onClick={() => setFilter(k)} className={cn("rounded-full px-2 py-0.5 text-[10px] font-bold transition sm:px-2.5 sm:py-1 sm:text-[11px]", filter === k ? "bg-primary text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200")}>
                {label}
              </button>
            ))}
          </div>
          <div className="inline-flex shrink-0 rounded-lg border border-slate-200 bg-white p-0.5">
            <button type="button" onClick={() => setView("grid")} className={cn("inline-flex items-center gap-0.5 rounded-md px-1.5 py-1 text-[10px] font-semibold sm:gap-1 sm:px-2 sm:py-1.5 sm:text-[11px]", view === "grid" ? "bg-primary text-white" : "text-slate-600")}><LayoutGrid className="h-3 w-3 sm:h-3.5 sm:w-3.5" /> Grid</button>
            <button type="button" onClick={() => setView("list")} className={cn("inline-flex items-center gap-0.5 rounded-md px-1.5 py-1 text-[10px] font-semibold sm:gap-1 sm:px-2 sm:py-1.5 sm:text-[11px]", view === "list" ? "bg-primary text-white" : "text-slate-600")}><List className="h-3 w-3 sm:h-3.5 sm:w-3.5" /> List</button>
          </div>
          <Button variant="outline" size="sm" className="h-7 shrink-0 px-2 text-[10px] lg:hidden sm:h-8 sm:text-xs" onClick={() => setShowAlertsMobile(true)}>
      ...
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(240px,280px)] lg:gap-4">
        <div>
          {attemptsQ.isLoading ? (
            ...
            <EmptyState icon={Radio} title="No active students" description="Students appear here while writing. Offline sessions leave after 3 minutes. Video is never saved." />
          ) : view === "grid" ? (
            <div className="grid grid-cols-2 gap-1.5 sm:gap-2 md:grid-cols-3 xl:grid-cols-4">
              {filtered.map((c) => (
                <StudentCard key={c.a.id} name={c.name} matric={c.matric} course={c.course} sev={c.sev} presence={c.presence} frameSrc={c.hasLiveVideo ? c.frame?.src : undefined} streamLive={c.hasLiveVideo} bars={c.bars} isDone={c.isDone} statusLabel={c.isDone ? doneStatusLabel(c.a.status) : undefined} onClick={() => setSelectedId(c.a.id)} />
              ))}
            </div>
          ) : (
            <ul className="space-y-1.5 sm:space-y-2">
              {filtered.map((c) => (
                <li key={c.a.id}>
                  <button type="button" onClick={() => setSelectedId(c.a.id)} className={cn("flex w-full items-center gap-2 rounded-lg border bg-white p-2 text-left shadow-sm transition hover:shadow-md sm:gap-3 sm:rounded-xl sm:p-3", severityBorderClass(c.sev))}>
                    <div className="relative h-10 w-14 shrink-0 overflow-hidden rounded-md bg-slate-900 sm:h-12 sm:w-16 sm:rounded-lg">
                      {c.isDone ? (
                        <div className="grid h-full place-items-center bg-sky-900/50"><CheckCircle2 className="h-5 w-5 text-sky-300" /></div>
                      ) : c.hasLiveVideo && c.frame?.src ? (
                        <img src={c.frame.src} alt="" className="h-full w-full object-cover" />
                      ) : (
                        <div className="grid h-full place-items-center"><UserRound className="h-4 w-4 text-white/30 sm:h-5 sm:w-5" /></div>
                      )}
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-xs font-bold text-slate-900 sm:text-sm">{c.name}</p>
                      <p className="truncate text-[10px] text-slate-500 sm:text-[11px]">{c.matric} · {c.course}</p>
                    </div>
                    {!c.isDone && <SignalBars bars={c.bars} className="hidden sm:flex" />}
                    <FaceChip presence={c.presence} sev={c.sev} isDone={c.isDone} statusLabel={c.isDone ? doneStatusLabel(c.a.status) : undefined} />
                    {!c.isDone && <span className="hidden font-mono text-[10px] font-semibold text-slate-600 sm:inline sm:text-[11px]">{formatDuration(c.presence.timeRemainingSec)}</span>}
                  </button>
                </li>
              ))}
            </ul>
      ...
      {showAlertsMobile && (
        <div className="fixed inset-0 z-[60] bg-black/40 lg:hidden" onClick={() => setShowAlertsMobile(false)}>
          <div className="absolute inset-x-0 bottom-0 max-h-[75dvh] overflow-y-auto rounded-t-2xl bg-white p-3 pb-[max(1rem,env(safe-area-inset-bottom))]" onClick={(e) => e.stopPropagation()}>
      ...
      {selected && (
        <div className="fixed inset-0 z-[70] flex justify-end bg-black/40" onClick={() => setSelectedId(null)}>
          <div className="flex h-full w-full max-w-md flex-col overflow-hidden bg-white shadow-2xl sm:max-w-lg" onClick={(e) => e.stopPropagation()}>
            <div className="sticky top-0 z-20 flex shrink-0 items-center gap-2 border-b border-slate-100 bg-white px-3 py-2.5 sm:px-4 sm:py-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="truncate text-[11px] text-slate-500">{selected.matric}{selected.isDone ? ` · ${doneStatusLabel(selected.a.status)}` : ""}</p>
              </div>
            ...
            <div className="sticky top-[52px] z-10 shrink-0 border-b border-slate-100 bg-slate-900 sm:top-[56px]">
              <div className="relative h-56 w-full sm:h-64 md:h-72">
                {selected.hasLiveVideo && selected.frame?.src ? (
                  <>
                    ...
                    <div className="absolute left-2 top-2 inline-flex items-center gap-1 rounded-full bg-red-600 px-2 py-0.5 text-[10px] font-bold text-white"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-white" /> LIVE</div>
                    <div className="absolute right-2 top-2 flex items-center gap-1.5 rounded-full bg-black/55 px-2 py-1">{selected.bars >= 2 ? <Wifi className="h-3 w-3 text-emerald-300" /> : <WifiOff className="h-3 w-3 text-red-300" />}<SignalBars bars={selected.bars} /></div>
                  </>
                ) : (
                  <>
                    <div className="absolute left-2 top-2 inline-flex items-center gap-1 rounded-full bg-slate-700 px-2 py-0.5 text-[10px] font-bold text-white">{selected.isDone ? doneStatusLabel(selected.a.status).toUpperCase() : selected.presence.cameraActive ? "WAITING FOR VIDEO" : "CAMERA OFF"}</div>
                    <div className="absolute right-2 top-2 flex items-center gap-1.5 rounded-full bg-black/55 px-2 py-1"><SignalBars bars={selected.bars} /></div>
                    <div className="flex h-full flex-col items-center justify-center gap-1.5 px-4 text-center text-white">
                      {selected.isDone ? <CheckCircle2 className="h-11 w-11 text-emerald-400/80" /> : selected.presence.cameraActive ? <UserRound className="h-11 w-11 opacity-40" /> : <CameraOff className="h-11 w-11 opacity-40" />}
                      <p className="text-sm font-semibold">{selected.isDone ? doneStatusLabel(selected.a.status) : faceLabel(selected.presence)}</p>
                      <p className="text-[10px] text-white/70">{selected.isDone ? "This student finished. Card leaves after 10 minutes." : selected.presence.cameraActive ? "Live frames appear when the student camera is streaming. Nothing is recorded." : "Student camera is off or unavailable. Offline cards leave after 3 minutes."}</p>
                    </div>
                  </>
                )}
              </div>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
              <div className="grid grid-cols-2 gap-2 border-b border-slate-100 p-3 text-sm sm:p-4">
                ...
                <Info label="Time left" value={selected.isDone ? "—" : formatDuration(selected.presence.timeRemainingSec)} />
                ...
                <Info label="Camera" value={selected.isDone ? "—" : selected.presence.cameraActive ? "Active" : "Off"} />
              ...
              {!selected.isDone && (
                <div className="flex flex-wrap gap-2 border-b border-slate-100 px-3 py-2.5 sm:px-4 sm:py-3">
                  <Button size="sm" variant="outline" className="h-8 border-amber-300 bg-amber-50 text-xs font-semibold text-amber-800 hover:bg-amber-100" disabled={warningBusy} onClick={() => void sendOfficerWarning()}>
                    {warningBusy ? <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> : <MessageSquareWarning className="mr-1.5 h-3.5 w-3.5" />}
                    Send warning
                  </Button>
                  <p className="w-full text-[10px] text-slate-400">Student gets a red alert instantly. Event is logged. Live video is never saved.</p>
                </div>
              )}
...
function FaceChip({ presence, sev, isDone, statusLabel }: { presence: ReturnType<typeof parsePresence>; sev: MonitorSeverity; isDone?: boolean; statusLabel?: string }) {
  ...
  return (
    <span className={cn("inline-flex max-w-full items-center gap-0.5 truncate rounded-full px-1.5 py-0.5 text-[9px] font-bold text-white sm:gap-1 sm:px-2 sm:text-[10px]", isAmberFace ? "bg-amber-500 text-white" : severityBadgeClass(sev))}>
      {!presence.cameraActive ? <CameraOff className="h-2.5 w-2.5 shrink-0 sm:h-3 sm:w-3" /> : sev === "normal" ? <CheckCircle2 className="h-2.5 w-2.5 shrink-0 sm:h-3 sm:w-3" /> : <AlertTriangle className="h-2.5 w-2.5 shrink-0 sm:h-3 sm:w-3" />}
...
function StudentCard({ name, matric, course, sev, presence, frameSrc, streamLive, bars, isDone, statusLabel, onClick }: { name: string; matric: string; course: string; sev: MonitorSeverity; presence: ReturnType<typeof parsePresence>; frameSrc?: string; streamLive?: boolean; bars: number; isDone?: boolean; statusLabel?: string; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} className={cn("overflow-hidden rounded-lg border bg-white text-left shadow-sm transition hover:shadow-md sm:rounded-xl sm:border-2", severityBorderClass(sev))}>
      <div className={cn("relative aspect-[5/4] sm:aspect-[4/3]", isDone ? "bg-gradient-to-br from-sky-800 via-slate-800 to-slate-900" : "bg-gradient-to-br from-slate-800 to-slate-900")}>
        {isDone ? (
          <div className="flex h-full flex-col items-center justify-center gap-1.5 bg-gradient-to-br from-sky-600/90 via-sky-800/80 to-slate-900 px-2 text-center">
            ...
            <p className="rounded-full bg-sky-500/90 px-2 py-0.5 text-[10px] font-extrabold uppercase tracking-wide text-white shadow sm:text-[11px]">{statusLabel || "Submitted"}</p>
          ...
          <div className="flex h-full items-center justify-center"><UserRound className="h-7 w-7 text-white/25 sm:h-10 sm:w-10" /></div>
        )}
        <span className="absolute left-1 top-1 inline-flex items-center gap-0.5 rounded-full bg-black/55 px-1 py-0.5 text-[8px] font-bold uppercase text-white sm:left-1.5 sm:top-1.5 sm:gap-1 sm:px-1.5 sm:text-[9px]">
          <span className={cn("h-1 w-1 rounded-full sm:h-1.5 sm:w-1.5", isDone ? "bg-sky-400" : streamLive ? "animate-pulse bg-red-500" : isOnline(presence.lastSeenAt) ? "animate-pulse bg-emerald-400" : "bg-slate-400")} />
          {isDone ? "Submitted" : streamLive ? "Live" : isOnline(presence.lastSeenAt) ? "Live" : "Off"}
        </span>
        {!isDone && (
          <div className="absolute right-1 top-1 rounded bg-black/55 px-1 py-0.5 sm:right-1.5 sm:top-1.5 sm:px-1.5 sm:py-1"><SignalBars bars={bars} /></div>
        )}
        <div className="absolute bottom-1 left-1 right-1 flex items-center justify-between gap-0.5 sm:bottom-1.5 sm:left-1.5 sm:right-1.5 sm:gap-1">
          ...
          {!isDone && <span className="shrink-0 rounded bg-black/50 px-1 py-0.5 font-mono text-[9px] font-semibold text-white sm:px-1.5 sm:text-[10px]">{formatDuration(presence.timeRemainingSec)}</span>}
        </div>
      </div>
      <div className="p-1.5 sm:p-2">
        <p className="truncate text-[11px] font-bold leading-tight text-slate-900 sm:text-xs sm:text-sm">{name}</p>
        <p className="truncate text-[9px] leading-tight text-slate-500 sm:text-[10px]">{matric} · {course}</p>
      </div>
    </button>
  );
}
      ...
      <ul className="max-h-[22rem] divide-y divide-slate-50 overflow-y-auto sm:max-h-[28rem]">
        {alerts.length === 0 ? (
          ...
          alerts.map((ev) => {
            ...
            return (
              <li key={ev.id}>
                <button type="button" onClick={() => onOpen(ev.student_id)} className={cn("flex w-full gap-2 px-3 py-2 text-left transition hover:bg-slate-50", !readIds.has(ev.id) && "bg-slate-50/50")}>
