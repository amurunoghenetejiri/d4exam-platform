...
function Page() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      if (!schoolName.trim() || !officialEmail.trim() || !applicantName.trim() || !applicantEmail.trim()) {
        setError("Please fill in school name, official email, applicant name, and applicant email.");
        return;
      }
      ...
      <div className="mx-auto max-w-xl space-y-6 px-4 py-10">
        ...
        <form onSubmit={submit} className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="space-y-1.5">
            ...
            <Input id="officialEmail" type="email" value={officialEmail} onChange={(e) => setOfficialEmail(e.target.value)} className="h-11" required />
          </div>
          <div className="space-y-1.5">
