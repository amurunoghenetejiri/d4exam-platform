...
type AttemptRow = {
  ...
  students: { full_name: string | null; matric_number: string | null } | null;
};
  ...
  const attemptsQ = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from("exam_attempts")
        .select(
          "id, exam_id, student_id, status, tab_switch_count, fullscreen_exit_count, total_score, security_review_status, submitted_at, examinations(title), students(full_name, matric_number)",
        )
                  ...
                  <button type="button" className="w-full text-left" onClick={() => setSelectedAttempt(a.id)}>
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <p className="text-sm font-bold text-slate-900">
                        {a.students?.full_name ?? "Student"} · {a.students?.matric_number ?? "—"}
                      </p>
