...
/** Drop document titles / section headers mis-detected as questions. */
function isNonQuestionStem(text: string): boolean {
  const s = text.trim();
  if (s.length < 8) return true;
  if (/multiple[- ]choice|test questions|question bank|answer key|instructions/i.test(s)) return true;
  if (/^[A-Z0-9][A-Z0-9\s\-/]{2,20}\s*[—–-]\s+/.test(s) && !/[?]/.test(s)) return true;
  if (!/[?]/.test(s) && s.split(/\s+/).length <= 14 && /\b(introduction|overview|chapter|unit)\b/i.test(s)) return true;
  return false;
}
...
export function validateDraft(q: DraftQuestion): string[] {
  ...
  else if (isNonQuestionStem(q.question_text)) errors.push("Looks like a title/header — not a question");
  ...
  // short_answer / numerical / essay: model answer optional (set during review)
  return errors;
}
