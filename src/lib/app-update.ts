...
export async function fetchAppVersionConfig(): Promise<AppVersionConfig> {
  ...
  try {
    ...
    console.warn("[app-update] config fetch failed", e);
...
export function resolveApkUrl(apkUrl: string): string {
  if (!apkUrl) return "/downloads/d4exam.apk";
  if (/^https?:\/\//i.test(apkUrl)) return apkUrl;
  if (typeof window !== "undefined") {
    try {
      return new URL(apkUrl, window.location.origin).href;
    } catch {
      /* fall through */
    }
  }
  return apkUrl.startsWith("/") ? apkUrl : `/${apkUrl}`;
}
...
export function isAndroidWebBrowser(): boolean {
  ...
  try {
    ...
    // Capacitor WebView already handled by isNativeShell
    return true;
  } catch {
    return false;
  }
