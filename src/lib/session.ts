...
export async function fetchSessionUser(): Promise<SessionUser | null> {
  ...
  const [profileByAuth, profileById, roleRes] = await Promise.all([
    supabase.from("profiles").select("id, full_name, email, status, school_id, auth_user_id").eq("auth_user_id", user.id).maybeSingle(),
    supabase.from("profiles").select("id, full_name, email, status, school_id, auth_user_id").eq("id", user.id).maybeSingle(),
