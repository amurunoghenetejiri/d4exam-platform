...
export async function switchToAccount(userId: string): Promise<{ ok: true } | { ok: false; error: string; needsLogin?: boolean }> {
  ...
  try {
    ...
    if (error) {
      return { ok: false, error: error.message || "Session expired. Sign in again.", needsLogin: true };
    }

    try {
      const { data: refreshed } = await supabase.auth.refreshSession();
      if (refreshed.session?.access_token && refreshed.session.refresh_token) {
        account.accessToken = refreshed.session.access_token;
        account.refreshToken = refreshed.session.refresh_token;
      }
    } catch {
      /* use existing tokens */
    }
