...
/** Decode via HTML Image when createImageBitmap fails on some PNG/JPG encodings. */
...
async function compressFromSource(
  ...
  try {
    ...
    const scale = Math.min(1, maxSide / Math.max(iw || 1, ih || 1));
    const w = Math.max(1, Math.round((iw || 1) * scale));
    const h = Math.max(1, Math.round((ih || 1) * scale));
...
async function fileToCompressedDataUrl(file: File): Promise<string> {
  try {
    ...
    return await compressFromSource(bitmap, file, bitmap);
  } catch {
    /* fall through to Image element */
  }
  const img = await fileToImageElement(file);
  return await compressFromSource(img, file);
}
...
export async function uploadSchoolLogo(opts: {
  ...
  try {
    ...
    // Raw data URL — skip decode entirely for moderate-sized files
    if (opts.file.size <= 900_000) {
      try {
        const raw = await fileToRawDataUrl(opts.file);
        if (raw.length <= 1_200_000) return { url: raw, path: "data-url-raw" };
      } catch {
        /* ignore */
      }
    }
    ...
    if (/decode|source image|cannot be read|Could not decode/i.test(msg)) {
      throw new Error(
        "This logo file cannot be read. Please use a clear PNG or JPG under 2MB (not HEIC or a damaged file). If exported from iPhone, open it and re-save as PNG or JPG.",
      );
    }
    throw new Error(msg);
  }
