...
export function CbtExamPage() {
  ...
  const onFaceSecurityEvent = useCallback(
    (ev: FaceSecurityEvent) => {
      ...
      if (aid) {
        void (async () => {
          try {
            ...
            await supabase
              .from("exam_attempts")
              .update({ tab_switch_count: next } as never)
              .eq("id", aid);
          } catch (e) {
            console.warn("[cbt] tab_switch_count update failed", e);
          }
        })();
      }
  ...
  useEffect(() => {
    ...
    const showOfficerWarning = (rawMsg?: string | null) => {
      ...
      const msg =
        ...
        "⚠️ Officer warning: Stay focused on the exam. Keep your face visible, do not leave the exam window, and follow all exam rules. Further violations may lead to your attempt being flagged.";
      toast.error(msg, {
        id: `officer-warn-${Date.now()}`,
        duration: 12000,
        description: "Swipe the toast left or right to dismiss.",
      });
    };
    ...
    const pushPresence = async () => {
      ...
      try {
        if (aid) {
          ...
          await supabase
            .from("exam_attempts")
            .update({ metadata: { ...prev, ...payload } as never } as never)
            .eq("id", aid);
        } else {
        ...
        console.warn("[live-monitor] presence update failed", e);
      }
    };

    void pushPresence();
    const t = window.setInterval(() => void pushPresence(), 12_000);
    return () => window.clearInterval(t);
