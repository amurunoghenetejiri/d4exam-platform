...
export function CbtExamPage() {
  ...
  async function finishAttempt(auto = false) {
    if (done) return;
    if (previewMode) { toast.message("Preview ended — nothing was saved"); setDone(true); return; }
    try {
      if (student?.studentId && examQ.data) {
        const status = auto ? "terminated" : "submitted";
        ...
        if (attemptId) {
          await supabase.from("exam_attempts").update({ status, submitted_at: new Date().toISOString(), answers } as never).eq("id", attemptId);
        }
        await supabase.from("results").upsert({
          exam_id: id, student_id: student.studentId, school_id: examQ.data.school_id,
          attempt_id: attemptId, status: "pending",
      ...
      <div className="grid min-h-dvh place-items-center bg-slate-50 p-4">
        <div className="w-full max-w-lg rounded-2xl border bg-white p-6 text-center shadow-sm">
          ...
          <div className="mt-6 flex flex-col gap-2 sm:flex-row sm:justify-center">
            {!previewMode && (<Button className="font-semibold" asChild><Link to="/student/results" search={{ id: exam.id }}>View Results</Link></Button>)}
