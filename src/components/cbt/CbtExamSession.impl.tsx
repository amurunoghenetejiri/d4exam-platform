...
export function CbtExamPage() {
  ...
  /** Pause camera stream when tab hidden; resume on return so officer sees camera off. */
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const showOfficerWarning = (rawMsg?: string | null) => {
      hapticOfficerWarning();
      const msg = (rawMsg && String(rawMsg).trim()) || "⚠️ Warning: Follow exam rules. Further violations may void your result.";
      toast.error(msg, { id: `officer-warn-${Date.now()}`, duration: 12000, description: "Swipe the toast left or right to dismiss." });
    };
    const channels: ReturnType<typeof supabase.channel>[] = [];
    if (studentId) {
      const ch = supabase.channel(`cbt-officer-warn-ie-${studentId}-${id}`)
        .on("postgres_changes", { event: "INSERT", schema: "public", table: "integrity_events", filter: `student_id=eq.${studentId}` }, (payload) => {
          ...
          showOfficerWarning(row.description);
        }).subscribe();
      channels.push(ch);
    }
    if (authUserId) {
      const ch = supabase.channel(`cbt-officer-warn-ntf-${authUserId}-${id}`)
        .on("postgres_changes", { event: "INSERT", schema: "public", table: "notifications", filter: `recipient_user_id=eq.${authUserId}` }, (payload) => {
          const row = payload.new as { type?: string; message?: string | null; title?: string | null; entity_id?: string | null };
          if (!row) return;
          ...
          showOfficerWarning(row.message || row.title);
        }).subscribe();
      channels.push(ch);
    }
