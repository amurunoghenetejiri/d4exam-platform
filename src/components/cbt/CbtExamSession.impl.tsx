...
export function CbtExamPage() {
  ...
  const questionsQ = useQuery({
    queryKey: ["cbt-questions", id, student?.studentId],
    enabled: Boolean(id),
    queryFn: async () => {
  ...
  useExamAttemptHeartbeat({
    ...
    active: started && !done && !previewMode,
  });
  ...
  async function finishAttempt(auto = false) {
    ...
    try {
      const paper = questions.map((qq, qi) => ({
        id: qq.id,
      ...
      const res = await saveCbtResult({
        ...
        durationSeconds:
          endsAtRef.current != null
            ? Math.max(0, Math.floor((Date.now() - (endsAtRef.current - (examQ.data?.duration_minutes || 0) * 60 * 1000)) / 1000))
            : null,
      });
