...
type ExamDetail = {
  ...
  course_id: string | null; school_id: string;
...
export function CbtExamPage() {
  ...
  const examQ = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("examinations")
        .select("id, title, status, duration_minutes, scheduled_start, scheduled_end, course_id, school_id, courses(code, name)")
        .eq("id", examId)
        .maybeSingle();
  ...
  const qsQ = useQuery({
    queryKey: ["cbt-qs", examId, examQ.data?.course_id],
  ...
  const questions = useMemo(() => {
    return (qsQ.data ?? []).map((q) => ({ ...q, options: decodeOptions(q.explanation) }));
  }, [qsQ.data]);
  ...
  if (done) {
    return (
      <div className="mx-auto max-w-lg space-y-4 p-6 text-center">
        ...
        <p className="text-sm text-slate-600">Your answers have been saved. Results appear after the examination officer releases them.</p>
        {resultId ? (
          <Button asChild className="font-semibold">
            <Link to="/student/results/$id" params={{ id: resultId }}>View result</Link>
          </Button>
        ) : (
      ...
      <div className="mx-auto max-w-3xl space-y-3 p-3 sm:p-4">
        <header className="flex items-center justify-between gap-3 rounded-xl bg-slate-900 px-3 py-2 text-white">
          <div className="flex items-center gap-2 min-w-0">
            <SchoolLogo logoUrl={school?.logoUrl} schoolName={school?.name ?? "School"} size="sm" className="ring-1 ring-white/20" />
          ...
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" className="h-8 bg-white text-slate-900 hover:bg-white/90" disabled={saving} onClick={() => void finish(true)}>
        ...
        {current && (
          <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="mb-3 flex items-center justify-between">
              ...
              <Button size="sm" variant="outline" className="h-8 text-xs" onClick={() => setFlagged((f) => ({ ...f, [current.id]: !f[current.id] }))}>
            ...
            <div className="mt-4 space-y-2">
              {current.options.map((opt, i) => {
                ...
                return (
                  ...
                  >
                    <span className={cn("grid h-7 w-7 shrink-0 place-items-center rounded-full border text-xs font-bold", selected ? "border-primary bg-primary text-white" : "border-slate-300")}>
            ...
            <div className="mt-6 flex justify-between">
              <Button size="sm" variant="outline" disabled={idx <= 0} onClick={() => setIdx((v) => Math.max(0, v - 1))}>
                <ChevronLeft className="mr-1 h-4 w-4" /> Previous
              </Button>
              <Button size="sm" className="font-semibold" disabled={idx >= total - 1} onClick={() => setIdx((v) => Math.min(total - 1, v + 1))}>
        ...
        <div className="rounded-xl border bg-white p-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {questions.map((q, i) => (
              <button key={q.id} type="button" onClick={() => setIdx(i)} className={cn("h-9 w-9 rounded-lg text-xs font-bold", i === idx ? "bg-primary text-white" : answers[q.id] != null ? "bg-slate-200" : "bg-slate-100")}>
                {i + 1}
              </button>
            ))}
          </div>
