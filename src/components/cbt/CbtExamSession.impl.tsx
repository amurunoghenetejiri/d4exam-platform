...
import { hapticOfficerWarning } from "@/lib/haptic";
...
export function CbtExamPage() {
  ...
  /** Instant red alert when officer sends WARNING_SHOWN for this student/exam. */
  useEffect(() => {
    if (previewMode || !started || done) return;
    const schoolId = String(examQ.data?.school_id ?? student?.schoolId ?? "");
    const studentId = student?.studentId;
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          // Stronger, longer haptic than face-detection alerts
