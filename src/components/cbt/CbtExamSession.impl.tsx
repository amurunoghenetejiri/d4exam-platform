...
export function CbtExamPage() {
  ...
  useEffect(() => {
    ...
    const ensureFullscreen = async () => {
      ...
      setFsGate(!ok);
      if (!ok) toast.message("Fullscreen is required — tap to continue in fullscreen");
    };
    ...
    const onReturn = () => {
      if (doneRef.current || !startedRef.current) return;
      if (document.visibilityState === "visible") void ensureFullscreen();
    };
    ...
    const tick = window.setInterval(() => {
      ...
      if (document.visibilityState === "visible" && !document.fullscreenElement) {
        setFsGate(true); void ensureFullscreen();
      }
    }, 2500);
    return () => {
  ...
  useEffect(() => {
    ...
    if (!schoolId || !studentId || !attemptId || !liveStream) return;
  ...
  useEffect(() => {
    if (previewMode || !started || done || !security.tabMonitoring) return;
    const schoolId = String(examQ.data?.school_id ?? student?.schoolId ?? session?.schoolId ?? "");
    const studentId = student?.studentId;
    if (!schoolId || !studentId || !id) return;
    let lastLog = 0;
  ...
  useEffect(() => {
    if (previewMode || !started || done || !security.requireCamera) return;
    const setTracksEnabled = (enabled: boolean) => {
      const stream = mediaStreamRef.current ?? liveStream;
      if (!stream) return;
      for (const t of stream.getVideoTracks()) {
        try { t.enabled = enabled; } catch { /* ignore */ }
      }
    };
    const onVis = () => {
      if (doneRef.current || !startedRef.current) return;
  ...
  useEffect(() => {
    ...
    const seenWarnIds = new Set<string>();
    let lastWarnAt = 0;

    const showOfficerWarning = (rawMsg?: string | null, eventId?: string | null) => {
      if (eventId && seenWarnIds.has(eventId)) return;
      if (eventId) seenWarnIds.add(eventId);
      // Debounce duplicate realtime + poll delivery
      const now = Date.now();
      if (now - lastWarnAt < 1_500) return;
      lastWarnAt = now;
      ...
      const msg =
        ...
        "⚠️ Warning from examination officer: Follow exam rules. Further violations may void your result.";
      toast.error(msg, {
        id: `officer-warn-${eventId || now}`,
        duration: 15_000,
        description: "From examination officer — stay focused on the exam.",
      });
    };
      ...
      const ch = supabase
        ...
        .on(
          ...
          (payload) => {
            ...
            if (!String(row.event_type || "").toUpperCase().includes("WARNING")) return;
            showOfficerWarning(row.description, row.id ?? null);
          },
        )
        .subscribe();
      channels.push(ch);
      ...
      const ch = supabase
        ...
        .on(
          ...
          (payload) => {
            ...
            if (ty !== "officer_warning" && !ty.includes("warning")) return;
            if (row.entity_id && row.entity_id !== id) return;
            showOfficerWarning(row.message || row.title, row.id ?? null);
          },
        )
        .subscribe();
      channels.push(ch);
    ...
    // Poll fallback — realtime can miss events under RLS / network
    let pollTimer: number | null = null;
    if (studentId) {
      const pollSince = new Date().toISOString();
      const poll = async () => {
        try {
          const { data } = await supabase
            .from("integrity_events")
            .select("id, event_type, description, exam_id, created_at")
            .eq("student_id", studentId)
            .gte("created_at", pollSince)
            .order("created_at", { ascending: false })
            .limit(8);
          for (const row of data ?? []) {
            const t = String((row as { event_type?: string }).event_type || "").toUpperCase();
            if (!t.includes("WARNING")) continue;
            const examId = (row as { exam_id?: string }).exam_id;
            if (examId && examId !== id) continue;
            const rid = String((row as { id: string }).id);
            if (seenWarnIds.has(rid)) continue;
            showOfficerWarning((row as { description?: string | null }).description, rid);
          }
        } catch {
          /* ignore */
        }
      ...
      void poll();
      pollTimer = window.setInterval(() => void poll(), 3_000);
    }
  ...
  useEffect(() => {
    ...
    const pushPresence = async () => {
      ...
      const camLive = Boolean(liveStream?.getVideoTracks().some((t) => t.readyState === "live" && t.enabled !== false));
      const tabHidden = typeof document !== "undefined" && document.visibilityState === "hidden";
      const payload = {
        cameraActive: !tabHidden && (camLive || fp.cameraActive),
        faceStatus: tabHidden ? "unavailable" : fp.faceStatus,
        ...
        videoStreaming: Boolean(!tabHidden && liveCamPublisherRef.current && camLive),
        tabHidden,
        // Keep name on every heartbeat so officer live-monitor can show it (profiles RLS)
        studentName: String(student?.fullName || "").trim() || null,
        matric: String(student?.matric || "").trim() || null,
      };
      try {
        if (aid) {
          const { data: row } = await supabase.from("exam_attempts").select("metadata").eq("id", aid).maybeSingle();
          const prev = (row?.metadata as Record<string, unknown> | null) ?? {};
          await supabase.from("exam_attempts").update({ metadata: { ...prev, ...payload } as never } as never).eq("id", aid);
        }
      } catch (e) { console.warn("[live-monitor] presence update failed", e); }
    };
    void pushPresence();
    const t = window.setInterval(() => void pushPresence(), 5_000);
    return () => window.clearInterval(t);
  }, [previewMode, started, done, student?.studentId, examQ.data?.school_id, id, liveStream, answers, questions.length, seconds]);

  async function requestSubmit() {
    if (done || finishingRef.current || previewMode) return;
    const unanswered = Math.max(0, TOTAL - answeredCount);
    const msg = unanswered > 0
      ? `You have ${unanswered} unanswered question${unanswered === 1 ? "" : "s"}. Submit this examination anyway? You cannot rewrite after submitting.`
      : "Submit this examination now? You cannot rewrite after submitting.";
    ...
    try {
      if (student?.studentId && examQ.data) {
        ...
        if (saved.error) toast.error(saved.error.message);
          ...
          toast.success(saved.published ? "Examination submitted — result is available now" : auto ? "Examination closed" : "Examination submitted successfully. Result will appear when released by the officer.");
        }
    ...
    try {
      ...
      if (needCam || needMic) {
        try {
          ...
          mediaStreamRef.current = stream;
          setLiveStream(stream);
          toast.success(needCam && needMic ? "Camera and microphone ready" : needCam ? "Camera ready" : "Microphone ready");
        } catch {
      ...
      if (!previewMode && student?.studentId && examQ.data?.school_id) {
        const nameMeta = {
          ...
          // Officers cannot always read profiles — write name onto the attempt
          studentName: String(student.fullName || "").trim() || null,
          matric: String(student.matric || "").trim() || null,
        };
        if (!attemptIdRef.current) {
          ...
          // Refresh name onto existing attempt so live-monitor can show it
          try {
            const { data: row } = await supabase.from("exam_attempts").select("metadata").eq("id", attemptIdRef.current).maybeSingle();
            const prev = (row?.metadata as Record<string, unknown> | null) ?? {};
            await supabase.from("exam_attempts").update({
              metadata: { ...prev, ...nameMeta },
  ...
  async function restoreFullscreenFromUser() {
    ...
    else toast.error("Could not enter fullscreen. Allow fullscreen for this site and try again.");
  }
  ...
  async function goToResult() {
    ...
    if (student?.studentId) {
      // Retry a few times — result row may lag right after submit
      for (let i = 0; i < 5 && !targetId; i++) {
      ...
      if (targetId) {
        ...
        resultIdRef.current = targetId;
      }
    }
    // Prefer result id; fall back to exam id ($id page resolves both)
      ...
      {fsGate && security.fullscreen && started && !done && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-950/90 p-4 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-2xl border border-slate-700 bg-white p-6 text-center shadow-2xl">
            ...
            <p className="mt-2 text-sm text-slate-600">This examination must stay in fullscreen. When you left the exam, fullscreen was closed. Tap below to continue in fullscreen.</p>
