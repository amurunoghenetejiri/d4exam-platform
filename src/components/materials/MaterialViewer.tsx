...
export function MaterialViewer({ item, siblings, courseLabel, onClose, onNavigate, onItemPatch }: Props) {
  ...
  const reader = (
    <div
      className="fixed inset-0 z-[200] flex flex-col bg-[#05080f] text-white"
      role="dialog"
      aria-modal
    ...
    >
      ...
      <div
        ref={wrapRef}
        className="relative flex min-h-0 flex-1 items-center justify-center overflow-hidden bg-[#05080f]"
      >
        {busy && (
        ...
        <div className="mx-auto flex max-w-3xl items-center justify-between gap-1 px-2 py-2 sm:px-4">
          ...
          <div className="flex items-center gap-0.5 sm:gap-1">
            <Button
              ...
              className="h-10 w-10 text-white hover:bg-white/10"
              onClick={() => {
                if (isImage(item)) setImgScale((s) => Math.max(0.5, +(s - 0.25).toFixed(2)));
                else setZoom((z) => Math.max(0.5, +(z - 0.15).toFixed(2)));
              }}
              aria-label="Zoom out"
            >
            ...
            <Button
              ...
              className="h-10 w-10 text-white hover:bg-white/10"
              onClick={() => {
                if (isImage(item)) setImgScale((s) => Math.min(5, +(s + 0.25).toFixed(2)));
                else setZoom((z) => Math.min(2.5, +(z + 0.15).toFixed(2)));
              }}
              aria-label="Zoom in"
            >
          ...
          <div className="flex items-center gap-0.5 sm:gap-1">
            {isImage(item) && (
              <Button
                type="button"
                size="sm"
                className="h-9 rounded-full bg-blue-600 px-3 text-xs font-semibold text-white hover:bg-blue-500"
            ...
            <div className="relative">
              ...
              {moreOpen && (
                <div className="absolute bottom-12 right-0 z-30 w-56 overflow-hidden rounded-xl border border-white/10 bg-[#0b1b3a] py-1 text-sm shadow-2xl">
                  ...
                  {item.description && (
                    <div className="border-t border-white/10 px-3 py-2 text-[11px] leading-snug text-white/50">
                      {item.description}
                    </div>
                  )}
                </div>
      ...
      {goPageOpen && (
        <div
          data-reader-chrome
          className="absolute inset-0 z-40 flex items-end justify-center bg-black/50 sm:items-center"
          onClick={() => setGoPageOpen(false)}
        >
          <div
            className="w-full max-w-sm rounded-t-2xl bg-[#0b1b3a] p-4 shadow-2xl sm:rounded-2xl"
        ...
        <div data-reader-chrome className="absolute inset-0 z-50 flex flex-col bg-[#0b1220]">
          <header className="flex shrink-0 items-center gap-2 border-b border-white/10 bg-[#0b1b3a] px-2 py-2">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[11px] text-white/55">
                {ocrBusy ? `${ocrStatusMsg} ${ocrProgress}%` : "Review and edit before saving"}
              </p>
            </div>
          </header>
          ...
          {!ocrBusy && (
            <div className="flex shrink-0 flex-wrap gap-2 border-t border-white/10 bg-[#0b1b3a] p-3">
              <Button type="button" variant="outline" className="border-white/20 bg-transparent text-white" onClick={() => setOcrOpen(false)}>
                Discard
              </Button>
              <Button
                type="button"
              ...
              <Button type="button" variant="secondary" onClick={() => openPrintableOcr(item.title, ocrDraft)}>
                Print / PDF
              </Button>
              <Button type="button" className="ml-auto" onClick={() => void saveOcr()} disabled={!ocrDraft.trim()}>
                Save conversion
              </Button>
            </div>
          )}
