...
public class ExamImmersivePlugin extends Plugin {
  ...
  public void vibrate(PluginCall call) {
    try {
      ...
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
        try {
          // Full amplitude on modern APIs for strong feedback
          if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            int[] amps = new int[pattern.length];
            for (int i = 0; i < pattern.length; i++) {
              // odd indices are "on" durations in classic vibrate API semantics when starting with 0 delay
              amps[i] = (i % 2 == 1) ? 255 : 0;
            }
            // If pattern starts with 0 delay then on, amps work; otherwise use default amplitude waveform
            boolean useAmps = pattern.length >= 2 && pattern[0] == 0;
            if (useAmps && Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
