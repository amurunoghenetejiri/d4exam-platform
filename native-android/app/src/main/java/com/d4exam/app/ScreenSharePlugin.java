 ...
 * stop() is ignored while keepAlive is true. Watchdog rebuilds VirtualDisplay if frames stop.
 */
...
public class ScreenSharePlugin extends Plugin {
        ...
        public void run() {
          try {
            if (sKeepAlive.get() && sMediaProjection != null) {
              ...
              if (needsRebuild) {
                Log.w(TAG, "watchdog rebuild capturing=" + sCapturing.get() + " ageMs=" + age);
                try {
  ...
  private void onScreenPermission(PluginCall call, ActivityResult result) {
    ...
    try {
      // Android 13+: request notification permission so FGS can post monitoring notification
      try {
      ...
      if (!ScreenCaptureService.FOREGROUND_READY.get()) {
        Log.w(TAG, "FGS not ready after await — proceeding anyway (check notification permission)");
      }
